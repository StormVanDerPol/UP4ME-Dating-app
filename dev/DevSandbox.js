...
class Haib extends Component {
    ...
    async componentDidMount() {
        ...
        let model = await nsfwjs.load(bundleResourceIO(require("../nsfwModel.json"), require('../nsfwWeights.bin')))

        console.log(model);

        this.setState({ modelReady: true });
        const imageAssetPath = Image.resolveAssetSource(require("../res/HotIndian.jpg"));

        console.log('brother please', imageAssetPath);
        ...
        var result = await model.classify(imageTensor);
    ...
    render() {
        return (
            ...
            <Fragment>
                <Image source={require("../res/pepe.jpg")} />
                <Text>tfready {this.state.tfReady + ''}</Text>
                <Text>modelReady {this.state.modelReady + ''}</Text>

                <Text>predready {this.state.predictionsReady + ''}</Text>
